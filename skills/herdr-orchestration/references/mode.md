...
2. Prefer ordinary routes by fit: **Pi Flash** (`pi-flash-*`, OpenRouter `google/gemini-3.8-flash`) and **Pi DeepSeek Flash** (`pi-dsflash-*`, OpenRouter `deepseek/deepseek-v4-flash-0731`) for fast cheap mechanical work; Grok or Cursor Auto when tools/evidence need them; premium models sparingly for justified escalation. Discover live agent names and pane IDs each time—do not hardcode pool layout. Judgment slots are authorized only under the coordinator profiles above; do not silently create unauthorized judgment workers. Do not broadcast routine work to the entire pool. On a quota or authentication failure, record it once, avoid retrying the unavailable route, and select an authorized alternative. Do not change subscriptions or credentials to recover.
